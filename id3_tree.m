function id3_tree()
    ...
    % ???? ????
    tree = buildID3Tree(X_train, y_train, features, featureNames);
...
function [bestFeature, gain] = chooseBestFeature(X, y)
    ...
    for i = 1:size(X, 2)
        ...
        for level = 1:length(featureLevels)
            ...
            newEntropy = newEntropy + (length(subset) / length(y)) * calculateEntropy(subset);
        end
...
function tree = buildID3Tree(X, y, features, featureNames)
    ...
    [bestFeature, ~] = chooseBestFeature(X, y);
    ...
    for level = 1:length(featureLevels)
        ...
        subtree = buildID3Tree(X(subsetIndex, :), y(subsetIndex), remainingFeatures, featureNames);
